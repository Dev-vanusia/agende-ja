...
const FormAuthenticate = ({ action }) => {
    ...
    const sendData = async (e) => {
        ...
        if (action === 'Register') {
            const isRegistered = await postRegister({ 
                nome: form.name.value, 
                senha: form.password.value, 
                mae: form.mother.value, 
                nascimento: form.birthdate.value, 
                cns: form.cns.value, 
                cpf 
                }, setLoading);
        ...
        <section className={`bg-white w-screen min-h-screen lg:min-h-0 lg:max-w-xl py-8 px-4 rounded-xl shadow-lg xl:rounded-tl-none xl:rounded-bl-none`}>
            ...
            <div>
                <form autoComplete='off' onSubmit={sendData}>
                    ...
                    <div>
                        {action === 'Login' ?
                            <p className='text-center font-poppins font-medium mt-4'>Ainda não tem uma conta? <Link to='/agende-ja/registro' className='text-lightM'>Cadastre-se.</Link></p>
                            :
                            <p className='text-center font-poppins font-medium mt-4'>Já tem uma conta? <Link to='/agende-ja/login' className='text-lightM'>Faça login.</Link></p>
                        }
                    </div>
                </form>
            </div>
